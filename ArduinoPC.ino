...
#define pgm_write_dword_near(address_short) __LDI_dword((uint16_t)(address_short))
...
{
  uint16_t toChange = pgm_read_dword_near(nearAddress);
  const PROGMEM uint16_t changed = ~toChange;
}
